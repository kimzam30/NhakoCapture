...
- [~] **T15 · Save page as PDF**: `chrome.debugger` attach →
      ...
      **Built.** The overlay tears itself down before the render, because
      `printToPDF` rasterises the live DOM and our scrim would be in it. Detach
      sits in a `finally`, tested — a stranded attachment would leave Chromium's
      debugging infobar up for the life of the tab.
...
- [~] **T20 · Docs correction**: rewrite `README.md` — the load path (it names
      ...
      **README done ahead of schedule at Kim's request** — rewritten with the
      icon, a hero screenshot generated by `tools/preview.mjs --hero` from the
      real code, a permissions table explaining `debugger`, and an honest
      "where it differs from Opera" section. `todo.md` still pending.
